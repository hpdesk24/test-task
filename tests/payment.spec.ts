...
test.describe("Payments - unauthorized user", () => {
  ...
  test("Should not proceed to monthly payment for unauthorized user", async ({
    page,
  }) => {
  ...
  test("Should not proceed to full payment for unauthorized user", async ({
    page,
  }) => {
...
test.describe("Payments redirects", () => {
  ...
  test("Should open correct payment screen for monthly subscription", async () => {
    await mainPage.monthlyPaymentButton.click();
    expect(await paymentsPage.totalAmount.innerText()).toEqual(
      "£20.00\nper\nmonth"
    );
  });
...
test.describe("Payment flow", () => {
  ...
  test("Should successfully complete payment", async () => {
    ...
    expect(
      await paymentSuccessPage.paymentSuccessText.isVisible()
    ).toBeTruthy();
  });
